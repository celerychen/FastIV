...
void conv2d_v1(ivf32* dst, int Wd, int Hd, int sd,
               ivf32* src, int Ws, int Hs, int ss, ivf32 coef[9]);

static int g_fail = 0;

/* 对一组参数：v1 与 v0 逐元素位级一致；并检查 dst 行尾 padding 未被写 */
static void run(const char* name, int Ws, int Hs, int Wd, int Hd,
    ...
    ivf32* d1  = (ivf32*)malloc(dst_n * sizeof(ivf32));
    srand(seed);
    for (size_t k = 0; k < src_n; k++) src[k] = ((ivf32)rand()/RAND_MAX)*2.0f - 1.0f;
    for (size_t k = 0; k < dst_n; k++) { d0[k] = -999.0f; d1[k] = -999.0f; }

    conv2d_v0(d0, Wd, Hd, sd, src, Ws, Hs, ss, coef);
    conv2d_v1(d1, Wd, Hd, sd, src, Ws, Hs, ss, coef);

    int mism = 0, overrun = 0;
    for (int j = 0; j < Hd; j++) {
        for (int i = 0; i < Wd; i++) {
            ivf32 a = d0[(size_t)j*sd+i], b = d1[(size_t)j*sd+i];
            /* 容差比较：v1 的 9 项展开在 -O2 下会被编译器做 FMA 收缩，
             * 与 v0 的分支循环相差最多 ~1 ULP，属数值等价而非逻辑差异 */
            if (fabsf(a - b) > 1e-4f * (fabsf(a) + 1.0f)) mism++;
        }
        for (int i = Wd; i < sd; i++)          /* 行尾 padding 槽 */
            if (d1[(size_t)j*sd+i] != -999.0f) overrun++;
    }
    ...
    free(src); free(d0); free(d1);
}

int main(void) {
    setvbuf(stdout, NULL, _IONBF, 0);
    printf("=== conv2d_v1 vs conv2d_v0 ===\n");
    ...
    /* 常规 same 尺寸，多种大小(含 1x1/2x2 退化，全部走边界路径) */
