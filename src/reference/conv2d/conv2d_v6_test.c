...
void conv2d_v6(ivf32* dst, int Wd, int Hd, int sd,
               ivf32* src, int Ws, int Hs, int ss, ivf32 coef[9]);

static int g_fail = 0;

/* 对一组参数：v6(NEON, j+=2+i+=8, 系数打包3向量+矢量×标量lane, 无 vext) 与 v0 逐元素容差一致；并检查 dst 行尾 padding 未被写(越界检测) */
static void run(const char* name, int Ws, int Hs, int Wd, int Hd,
    ...
    conv2d_v6(d2, Wd, Hd, sd, src, Ws, Hs, ss, coef);
...
int main(void) {
    ...
    printf("=== conv2d_v6 (NEON, j+=2 vertical reuse) vs conv2d_v0 ===\n");
    ...
    /* 专门触发 8-wide 余数路径：内部宽=7(0个NEON块,全标量收尾) / 13 / 15 */
    run("i8edge 9x9",        9,9, 9,9, 9,9, sobel, 44);   /* 内部宽=7 */
    run("i8edge 15x15",      15,15, 15,15, 15,15, box, 45); /* 内部宽=13 = 1块+5收尾 */
    run("i8edge 17x17",      17,17, 17,17, 17,17, rnd, 46); /* 内部宽=15 = 1块+7收尾 */
