...
void conv2d_v4(ivf32* dst, int Wd, int Hd, int sd,
               ivf32* src, int Ws, int Hs, int ss, ivf32 coef[9]);

static int g_fail = 0;

/* 对一组参数：v4(NEON, j+=2 纵向复用) 与 v0 逐元素容差一致；并检查 dst 行尾 padding 未被写(越界检测) */
static void run(const char* name, int Ws, int Hs, int Wd, int Hd,
    ...
    conv2d_v4(d2, Wd, Hd, sd, src, Ws, Hs, ss, coef);
...
int main(void) {
    ...
    printf("=== conv2d_v4 (NEON, j+=2 vertical reuse) vs conv2d_v0 ===\n");
