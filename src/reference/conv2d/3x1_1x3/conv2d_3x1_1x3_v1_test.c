...
void conv2d_3x1_1x3_v1(ivf32* dst, int Wd, int Hd, int sd,
...
static void run(const char* name, int Ws, int Hs, int Wd, int Hd,
    ...
    conv2d_3x1_1x3_v1(d_oop, Wd, Hd, sd, src, Ws, Hs, ss, sep6);          /* 可分离 OOP(被测) */
    ...
    if (ss == sd && Ws == Wd && Hs == Hd) {
        ...
        conv2d_3x1_1x3_v1(buf, Wd, Hd, sd, buf, Ws, Hs, ss, sep6);   /* src==dst */
        int ip_mism = 0;
        for (int j = 0; j < Hd; j++)
            for (int i = 0; i < Wd; i++) {
...
int main(void) {
    ...
    printf("=== conv2d_3x1_1x3_v1 (separable, 6 系数, 去大buffer/in-place/一次4行) vs conv2d_v0 (外积构造的 3x3 核) ===\n");
