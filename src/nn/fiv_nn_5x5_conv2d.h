 ...
 *
 * 5x5 kernel plane convolution (STD fast path, stride 2 only).
 */

#ifndef _FIV_NN_5X5_CONV2D_H_
#define _FIV_NN_5X5_CONV2D_H_
...
/* Dense (STD) 5x5 stride-2 conv over all (oc, ic): d[oc] = sum_ic conv(s[ic],
   w[oc][ic]). Owns the oc / ic loops so the module can be tuned in place.
   zero_pad: 0 = replicate edge, 1 = zero padding.
   pt / pl: explicit start pads (SAME padding -> 1/1). */
...
/* Single src plane x single 5x5 coef -> one half-sized dst plane (stride 2). */
